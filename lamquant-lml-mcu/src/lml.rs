...
/// build" error (see the decode arms above). An earlier version of this
/// comment claimed host decoders always support it; that was never true
/// of the code, and it matters, because it is the sentence someone
/// would rely on when deciding whether writing such an archive is safe
/// for other readers. It is not: only a matching build can read it.
///
