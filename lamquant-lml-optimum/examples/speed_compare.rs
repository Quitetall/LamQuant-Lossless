...
fn main() {
    ...
    let t = sig[0].len();
    let windows: Vec<Vec<Vec<i64>>> = (0..t)
        .step_by(W)
        .map(|s| {
            let e = (s + W).min(t);
            sig.iter().map(|c| c[s..e].to_vec()).collect()
        })
        .collect();
    let t0 = Instant::now();
    let mut fb = 0;
    for w in &windows {
        fb += LmlCodec.encode(w, Mode::Lossless).unwrap().len();
    }
    let ft = t0.elapsed().as_secs_f64();
    let t0 = Instant::now();
    let mut ob = 0;
    for w in &windows {
        ob += LmoCodec.encode(w, Mode::Lossless).unwrap().len();
    }
    let ot = t0.elapsed().as_secs_f64();
    println!(
        "  {:<28} floor {:.2}s ({} B) | optimum {:.2}s ({} B)",
