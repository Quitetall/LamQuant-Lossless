...
fn pick_refs(sig: &[Vec<i64>], i: usize, r: usize) -> Vec<usize> {
    ...
    let mut scored: Vec<(f64, usize)> = (0..i)
        .map(|j| {
            let mu_j = sig[j].iter().map(|&v| v as f64).sum::<f64>() / t as f64;
            let mut cov = 0.0;
            for k in 0..t {
                cov += (sig[i][k] as f64 - mu_i) * (sig[j][k] as f64 - mu_j);
            }
            (cov.abs(), j)
        })
        .collect();
...
fn fit_ls(tgt: &[i64], refs: &[&[i64]], lo: usize, hi: usize) -> Option<Vec<f64>> {
    ...
    for d in 0..r {
        ata[d][d] += 1e-6 * tr.max(1.0);
    }
    // Gaussian elimination
    for c in 0..r {
        let mut piv = c;
        for rr in (c + 1)..r {
            if ata[rr][c].abs() > ata[piv][c].abs() {
                piv = rr;
            }
        }
        if ata[piv][c].abs() < 1e-12 {
            return None;
        }
        ata.swap(c, piv);
        atb.swap(c, piv);
        ...
        if s < b {
            ...
            match coeffs {
                Some(c) => {
                    for k in s..e {
                        let mut pred = 0.0;
                        for (a, rf) in refs.iter().enumerate() {
                            pred += c[a] * rf[k] as f64;
                        }
...
fn main() {
    println!(
        "# SHOT 1 — per-block backward-adaptive cross-channel LS (HHI's referential mechanism)\n"
    );
    ...
    for path in std::env::args().skip(1) {
        ...
        println!(
            "  {:<8} {:<5} {:>12} {:>9} {:>11} {:>11} {:>6}",
            "block", "refs", "bytes", "bps", "vs floor", "vs base", "rt"
        );
