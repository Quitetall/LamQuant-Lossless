...
fn main() {
    let mut wins: BTreeMap<(usize, usize), usize> = BTreeMap::new();
    for path in std::env::args().skip(1) {
        ...
        while start < t {
            ...
            for (ci, &(l, r, m)) in CFG.iter().enumerate() {
                for seg in 0..2 {
    ...
    println!(
        "  # configs absent here are candidates to PRUNE (never win ⇒ never-worse preserved)."
    );
}
