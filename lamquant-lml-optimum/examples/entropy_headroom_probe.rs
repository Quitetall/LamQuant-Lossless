...
#[inline]
fn log2bucket(v: i64) -> u32 {
    64 - (v.unsigned_abs().max(1)).leading_zeros()
}
...
fn main() {
    ...
    for path in std::env::args().skip(1) {
        ...
        println!(
            "## {} ({}ch x {})  — scale_cond REALIZED = {:.4} bps",
            name, nch, t, realized_bps
        );
        for (lbl, bps) in rows {
            let gap = 100.0 * (bps - realized_bps) / realized_bps;
            println!(
                "   {:<38} {:.4} bps   ({:+.2}% vs realized{})",
                lbl,
                bps,
                gap,
                if bps < realized_bps {
    ...
    println!(
        "# joint loop cannot win via a richer coder (mv_rls already whitens; predictor is joint)."
    );
}
