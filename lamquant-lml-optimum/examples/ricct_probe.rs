...
#[derive(Clone, Copy)]
struct Rot { p: usize, q: usize, theta: f64 }

/// Mean-removed population channel covariance (n x n).
fn covariance(sig: &[Vec<i64>]) -> Vec<Vec<f64>> {
    let n = sig.len();
    let t = sig[0].len();
    let mus: Vec<f64> = sig.iter().map(|c| c.iter().map(|&v| v as f64).sum::<f64>() / t as f64).collect();
    let mut cov = vec![vec![0.0f64; n]; n];
    for i in 0..n {
        for j in i..n {
            let mut s = 0.0;
            for k in 0..t { s += (sig[i][k] as f64 - mus[i]) * (sig[j][k] as f64 - mus[j]); }
            let c = s / t as f64;
            cov[i][j] = c; cov[j][i] = c;
        }
    }
...
fn lift_rotate(a: &mut [i64], b: &mut [i64], phi: f64) {
    let (c, s) = (phi.cos(), phi.sin());
    if s.abs() < 1e-15 { return; } // ~identity (phi≈0 or pi); skip (no-op kept identical on decode)
    let t = (c - 1.0) / s;
    for k in 0..a.len() { a[k] += rnd(t * b[k] as f64); }
    for k in 0..a.len() { b[k] += rnd(s * a[k] as f64); }
    for k in 0..a.len() { a[k] += rnd(t * b[k] as f64); }
}
...
fn ricct_perwindow_floor(sig: &[Vec<i64>]) -> (usize, usize) {
    ...
    while s < t {
        ...
        bytes += LmlCodec.encode(&win, Mode::Lossless).map(|x| x.len()).unwrap_or(0);
        side += rots.len() * 10;
        s = e;
    }
    (bytes, side)
...
fn main() {
    ...
    for path in std::env::args().skip(1) {
        ...
        if nch < 2 { println!("# {} ({}ch) — skip (need >=2ch)", name, nch); continue; }
        ...
        let per_var_o = orig.iter().map(|c| { let t = c.len() as f64; let mu = c.iter().map(|&v| v as f64).sum::<f64>()/t; c.iter().map(|&v|{let z=v as f64-mu; z*z}).sum::<f64>()/t }).collect::<Vec<_>>();
        let per_var_r = rot.iter().map(|c| { let t = c.len() as f64; let mu = c.iter().map(|&v| v as f64).sum::<f64>()/t; c.iter().map(|&v|{let z=v as f64-mu; z*z}).sum::<f64>()/t }).collect::<Vec<_>>();
        ...
        println!("  rotations={} (side~{}B={:.3}% of base)  rt={}", rots.len(), side, 100.0*side as f64/base as f64, if rt_ok {"OK"} else {"FAIL!!"});
        println!("  LmoCodec base (mv_rls) = {:>10} ({:.4} bps)", base, base as f64*8.0/nm);
        println!("  LmoCodec on RICCT      = {:>10} ({:.4} bps)  + side = {:>10}  => {:+.2}% vs base", rric, rric as f64*8.0/nm, total, dpct);
        println!("  --- isolate the spatial stage (no-spatial floor) ---");
        println!("  LmlCodec floor orig    = {:>10}", floor_o);
        println!("  LmlCodec floor RICCT   = {:>10}  ({:+.2}% vs floor_orig ⇒ {} )", floor_r, floor_dec, if floor_dec<0.0 {"rotation DECORRELATES"} else {"rotation HURTS floor"});
        println!("  RICCT-spatial + side vs adaptive mv_rls base: {:+.2}%", vs_base);
        ...
        println!("  cascade proxy (Σ entropy(Δ) bytes):  orig={:>10}  rotated={:>10}  ({:+.1}%)", cod_o, cod_r, 100.0*(cod_r as f64-cod_o as f64)/cod_o as f64);
        println!("  energy in top channel:                orig={:.3}     rotated={:.3}", top_o, top_r);
        println!();
    }
