...
use lamquant_abir_montage::{
    channel_spec_for, coordinate_frame_for, coordinate_from_f32, montage_root_frame, MontageError,
};
...
use semantic_abir::{
    ...
    ChannelBasisTag, ChannelSpec, Clock, ClockTag, ConceptId, ContentId, CoordinateFrame,
    DatasetDraft, DatasetTag, DecodedSemantics, ElementType, EncodedBlock, Layout, ObjectId,
    PayloadDescriptor, Presence, Rational, Recording, RecordingTag, ReferenceKind, SourceCapsule,
    SourceKey, Stream, StreamTag, ValidationLimits,
};
...
const BASE_CHANNEL_WORKSPACE_BYTES: usize = 512;
// One coordinate frame carries 16 exact transform values plus uncertainty.
// Canonical tagged numerics can contain two signed i128 strings (40 bytes each)
// plus keys and punctuation. Add retained semantic structs and fixed catalog
// overhead so preflight rejects expansion before projection allocates it.
const TAGGED_EXACT_NUMBER_JSON_BYTES: usize = 160;
const COORDINATE_FRAME_EXACT_NUMBER_COUNT: usize = 17;
const COORDINATE_FRAME_JSON_FIXED_BYTES: usize = 1_024;
const LOCATED_CHANNEL_WORKSPACE_BYTES: usize = BASE_CHANNEL_WORKSPACE_BYTES
    + size_of::<CoordinateFrame>()
    + size_of::<ChannelSpec>()
    + COORDINATE_FRAME_EXACT_NUMBER_COUNT * TAGGED_EXACT_NUMBER_JSON_BYTES
    + COORDINATE_FRAME_JSON_FIXED_BYTES;
...
pub enum LmqcPayloadKind {
    /// Half-precision continuous latent tensor.
    Fp16Latent,
    /// Finite scalar-quantizer token tensor.
    FsqTokens,
}
...
pub struct LmqcBundleInput {
    ...
    /// Declared reconstruction-fidelity contract for this lossy source.
    pub fidelity: CodecFidelity,
    /// Codec implementation identity used to produce the neural payload.
    pub implementation: CodecImplementation,
    /// Checkpoint and PCCP evidence for the neural payload producer.
...
pub enum LmqcBundleError {
    /// BCS2 catalog, framing, or resource-bound failure.
    Bundle(CodecBundleError),
    /// Legacy LMQC framing or integrity failure.
    Legacy(LmqcError),
    /// Electrode-coordinate projection failure.
    Montage(MontageError),
    /// Well-formed legacy feature unsupported by this migration contract.
    InvalidLegacy(&'static str),
    /// Canonical ABIR debug representation could not be encoded.
    SemanticEncoding,
    /// BCS2 catalog, packets, and projected ABIR semantics disagree.
    SemanticMismatch,
    /// Projected ABIR dataset violates semantic validation limits.
    SemanticValidation,
}
...
pub struct LmqcMetadata {
    /// Legacy container version.
    pub version: u8,
    /// Decoded signal channel count.
    pub n_channels: u16,
    /// Neural latent channel count.
    pub latent_c: u16,
    /// Neural latent temporal-frame count.
    pub latent_t: u16,
    /// Decoded sample rate in hertz.
    pub sample_rate: u16,
    /// Decoded samples per channel.
    pub window_samples: u32,
    /// Semantic neural payload discriminator.
    pub payload_kind: LmqcPayloadKind,
    /// Optional flat XYZ electrode coordinates in metres, three values per channel.
    pub coords: Option<Vec<f32>>,
    /// Optional channel labels in wire order.
    pub channels: Option<Vec<alloc::string::String>>,
    /// Borrowed neural payload length in bytes.
...
impl<'a> OpenedLmqcBcs2<'a> {
    /// Canonical ABIR dataset proven to describe this packet bundle.
    ...
    pub const fn metadata(&self) -> &LmqcMetadata {
        &self.metadata
    }
    ...
    #[deprecated(
        since = "0.1.0",
        note = "use metadata(); no container payload is retained"
    )]
    pub const fn container(&self) -> &LmqcMetadata {
        self.metadata()
    }

    /// Validated underlying semantic codec bundle.
...
fn payload_start_from_prefix(
    ...
    let flags = bytes[5];
    let per_channel_workspace = if flags & FLAG_COORDS != 0 {
        LOCATED_CHANNEL_WORKSPACE_BYTES
    } else {
        BASE_CHANNEL_WORKSPACE_BYTES
    };
    let channel_workspace = usize::from(n_channels)
        .checked_mul(per_channel_workspace)
        .ok_or(LmqcBundleError::InvalidLegacy(
            "channel workspace exceeds catalog bound",
        ))?;
    if channel_workspace > bounds.max_catalog_bytes as usize {
        return Err(LmqcBundleError::InvalidLegacy(
            "channel workspace exceeds catalog bound",
        ));
    }
...
fn dataset_from_lmqc(
    ...
    let (montage_digest, montage_root) = if container.coords.is_some() {
    ...
    for index in 0..usize::from(container.n_channels) {
        let frame = match (&container.coords, montage_digest) {
            (Some(coordinates), Some(digest)) => {
                let offset = index
                    .checked_mul(3)
                ...
                let xyz = coordinates
                    .get(offset..offset + 3)
                    .ok_or(LmqcBundleError::SemanticValidation)?;
                coordinate_frame_for(
                    &digest,
                    u32::try_from(index).map_err(|_| LmqcBundleError::SemanticValidation)?,
                    coordinate_from_f32([xyz[0], xyz[1], xyz[2]])
                        .map_err(LmqcBundleError::Montage)?,
                    montage_root.as_ref().map(|root| root.id()),
                    coordinate_uncertainty,
                )
                .map_err(LmqcBundleError::Montage)?
            }
            (None, None) => None,
            _ => return Err(LmqcBundleError::SemanticValidation),
        };
...
fn legacy_parameters(
    ...
    vec![
        text_parameter("abir.revision", super::LINKED_ABIR_REVISION),
