...
#[test]
#[ignore = "explicitly regenerates committed wire fixtures"]
fn regenerate_frozen_lmqc_fixtures() {
    use std::fmt::Write as _;
    use std::path::Path;

    let coordinates = [0.08, -0.0725, 0.03409375, -0.08, 0.0725, -0.03409375];
    let unknown_second = [0.08, -0.0725, 0.03409375, f32::NAN, f32::NAN, f32::NAN];
    let long_names = ["Fp1-Ref".to_string(), "Fp2-Ref".to_string()];
    let short_names = ["Fp1".to_string(), "Fp2".to_string()];
    let cases = [
        (
            "fp16_none.hex",
            encode_lmqc(
        ...
        (
            ...
            encode_lmqc(
                ...
                Some(&coordinates),
                Some(&short_names),
                &[0xde, 0xad, 0xbe, 0xef, 0x55],
            )
            .unwrap(),
        ),
    ];
    let fixture_dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/lmqc");
    for (name, bytes) in cases {
        ...
        encoded.push('\n');
        std::fs::write(fixture_dir.join(name), encoded).unwrap();
    }
}
...
fn frozen_lmqc_fixtures_restore_byte_exact_across_payload_and_montage_forms() {
    for (hex, sha256, payload_kind, has_coords, has_names) in FROZEN_FIXTURES {
        ...
        assert_eq!(opened.metadata().payload_kind, payload_kind);
        assert_eq!(opened.metadata().coords.is_some(), has_coords);
        assert_eq!(opened.metadata().channels.is_some(), has_names);
        assert_eq!(
...
fn legacy_lmqc_converts_to_semantic_bundle_and_restores_byte_exact() {
    ...
    assert_eq!(opened.metadata().n_channels, 2);
    assert_eq!(opened.metadata().latent_c, 32);
    assert_eq!(opened.metadata().latent_t, 79);
    assert_eq!(opened.metadata().sample_rate, 250);
    assert_eq!(opened.metadata().window_samples, 2_500);
...
fn unknown_flags_and_reserved_bytes_fail_preflight_but_high_channel_counts_import() {
    ...
    let maximum_channel_count = encode_lmqc(
        u16::MAX,
        32,
        79,
        250,
    ...
    .unwrap();
    let constrained = ResourceBounds {
        max_catalog_bytes: u32::from(u16::MAX),
        ..ResourceBounds::default()
    };
    assert!(matches!(
        lmqc_to_bcs2(&maximum_channel_count, import_policy(), constrained),
        Err(LmqcBundleError::InvalidLegacy(
            "channel workspace exceeds catalog bound"
        ))
    ));

    let located_channel_count = 32_768_u16;
    let located_coordinates = vec![0.0_f32; usize::from(located_channel_count) * 3];
    let located_amplification = encode_lmqc(
        located_channel_count,
        32,
        79,
        250,
        2_500,
        PAYLOAD_FP16_LATENT,
        Some(&located_coordinates),
        None,
        &[0x11],
    )
    .unwrap();
