...
impl OpSpec {
    ...
    ///
    /// Always prepends `--emit-plan-projections` so the lml binary streams
    /// graph-bound observations back to the supervising plan runner.
    pub fn build_args(&self, input: Option<&str>, output: Option<&str>) -> Vec<String> {
        // `--emit-plan-projections` is a global flag on the lml CLI; it
        // must come BEFORE the subcommand (clap rejects it otherwise).
        let mut args: Vec<String> =
            vec!["--emit-plan-projections".to_string(), self.cmd.to_string()];
        if let Some(i) = input {
...
pub const CODEC_OPERATION_IDS: &[&str] = &[
    "encode_lma",
    "encode_lml_siblings",
    "decode",
    "verify",
