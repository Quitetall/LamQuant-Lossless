...
fn stale_source_capsule_cannot_authorize_semantic_equivalence() {
    ...
    let mut options = SemanticLoweringOptions::default().with_limits(ValidationLimits::default());
    options = options.with_explicit_capsules(vec![stale]);
    let remapped = from_owned_uniform_signal(
