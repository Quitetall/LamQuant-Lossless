...
//! byte source and a semantic-lowering method that
...
//! description exactly the way a hand-written reader would.
...
//!     `EeglabReader`'s float-bitcast lossless path). A *data-driven* descriptor
...
pub struct FormatDescriptor {
    /// Becomes ABIR's `source.format` recording key (e.g. `"RAW"`).
    ...
    /// resulting content-addressed source capsules. Recognized values today:
...
fn resolve_channel_count(cc: &ChannelCount, obj: &JsonObject) -> Result<usize, DescriptorError> {
    let n = match cc {
...
/// Decode straight to the codec's `i64` native working currency.
...
/// name), producing validated semantic ABIR. Panic-free on
...
pub fn lower_descriptor_to_abir(
    ...
    lower_descriptor_to_abir_with_options(
        descriptor,
        data,
        sidecar_bytes,
...
/// Interpret a descriptor while applying caller-selected semantic modality,
/// events, limits, and source binding in the same lowering transaction.
pub fn lower_descriptor_to_abir_with_options(
    descriptor: &FormatDescriptor,
    data: &[u8],
    sidecar_bytes: &[u8],
...
mod tests {
    ...
    fn raw_sidecar_json(n_ch: usize, sample_rate: f64, dtype: &str, orient: &str) -> String {
        ...
        let pmin: Vec<String> = (0..n_ch)
            .map(|i| format!("{}", -200.0 - i as f64))
            .collect();
    ...
    fn synth_value(s: usize, ch: usize) -> i32 {
        (s as i32)
            .wrapping_mul(37)
            .wrapping_add((ch as i32).wrapping_mul(911))
            % 60_000
            - 30_000
    }
    ...
    fn endian_big_decodes_differently_from_little() {
        ...
        let le_signal = le_read.uniform_signal().unwrap();
        let be_signal = be_read.uniform_signal().unwrap();
    ...
    fn reciprocal_sample_rate_derives_hz_and_rejects_zero_divisor() {
        ...
        assert!(
            (signal.sample_rate() - 250.0).abs() < 1e-9,
            "sr={}",
            signal.sample_rate()
        );
    ...
    fn sidecar_keys_preserve_payload_and_sidecar_bytes() {
        ...
        assert_eq!(signal.sidecar_first("sidecar_json").unwrap().bytes, sidecar);
        assert_eq!(signal.sidecar_first("payload").unwrap().bytes, payload);
    }
    ...
    fn i24_dtype_decodes_correctly() {
        ...
        assert_eq!(
            read.uniform_signal().unwrap().signal()[0],
            vec![0x7F_FFFF_i64, -1, -8_388_608]
        );
    }
}
