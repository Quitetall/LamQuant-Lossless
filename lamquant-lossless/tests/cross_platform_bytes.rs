...
fn container_bytes_deterministic_smoke() {
    ...
    assert_eq!(
        sha256_hex(&second),
        hash,
        "semantic container encode must be byte-deterministic across two invocations"
    );
}
...
fn container_decode_bit_exact_after_repeated_encode() {
    ...
    let sink1 =
        support::encode_uniform_signal(&sig, 250.0, 128, "{}", LpcMode::default()).into_bytes();
    let sink2 =
        support::encode_uniform_signal(&sig, 250.0, 128, "{}", LpcMode::default()).into_bytes();
    assert_eq!(
        sink1, sink2,
...
fn container_profile_invariants() {
    ...
    let sink =
        support::encode_uniform_signal(&sig, 250.0, 64, "{}", LpcMode::default()).into_bytes();
    assert_eq!(
        &sink[0..4],
        b"ABIR",
        "semantic container encode must emit the current ABIR/BCS2 bundle"
    );
