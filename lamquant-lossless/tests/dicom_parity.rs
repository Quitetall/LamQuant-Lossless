...
fn parse_12lead_ecg_fixture_matches_pydicom_golden() {
    ...
    let b = read
        .uniform_signal()
        .expect("12-lead fixture should expose uniform signal");
    ...
    assert_eq!(b.source_metadata().format, "DICOM_WAVEFORM");
...
fn parse_general_ecg_fixture_matches_synthetic_golden() {
    ...
    let b = read
        .uniform_signal()
        .expect("general ECG fixture should expose uniform signal");
...
fn bundle_validates() {
    let mut reader = DicomWaveformReader::new(fixture("12lead_ecg.dcm"));
    let b = reader.lower_to_abir().unwrap();
    assert!(b.uniform_signal().is_some());
}
