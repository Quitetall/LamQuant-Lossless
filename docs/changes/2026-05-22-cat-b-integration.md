...
| `loom 0.7` | `lamquant-core` dev-dep | always (dev) | 1/1 pass (`loom_atomic_increment_smoke`) | No production shared-state code today. Available for future dual-core RP2350 work + any non-trivial Arc/Mutex/atomic interleaving. |
| `rkyv 0.7` | `lamquant-core` dev-dep | always (dev) | 1/1 pass (`rkyv_struct_archive_smoke`) | Zero-copy mmap'd archive. Trigger condition: conformance asset corpus > 200 MB. Currently a few MB; bookmark. Smoke proves the archive/deserialize roundtrip works through serde-like derives. |
| `bitstream-io 4` | `lamquant-core` dev-dep | always (dev) | 1/1 pass (`bitstream_io_roundtrip_smoke`) | Bit-level stream r/w. Audit recommended over `bitvec` as single source of truth. Available when we need a bit-packed format outside the codec's golomb-rice path. |
| `faer 0.22` | `lamquant-core` dev-dep | always (dev) | 1/1 pass (`faer_4x4_multiply_smoke`) | Pure-Rust dense linear algebra. Watch-list — no dense linalg in current scope. Smoke verifies the Mat / index API is reachable on host. |
