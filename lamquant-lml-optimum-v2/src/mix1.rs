...
impl Mix1Codec {
    ...
    pub fn decode_window(&self, packet: &[u8]) -> Result<Mix1Decoded, OptimumV2Error> {
        ...
        let (score_shift, side) =
            mix1_lattice::parse_side(&frame.graph, frame.channels, frame.samples)?;
        let residuals = mix1_entropy::decode(
            &frame.payload,
            frame.event_count,
            frame.channels,
            frame.samples,
        ...
        let samples = decode_samples(&residuals, score_shift, &side, frame.bit_depth)?;
